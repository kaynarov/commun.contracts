...
void commun_social::createreput(name account) {
...
void commun_social::changereput(name voter, name author, int64_t rshares) {
    require_auth(_self);
...
void commun_social::deletereput(name account) {
    require_auth(account);

    tables::reputation_singleton reputation_tbl(_self, account.value);
    auto acc_rep = reputation_tbl.get_or_default();
    check(acc_rep.reputation != 0, "The reputation has already removed");
    reputation_tbl.remove();
}

EOSIO_DISPATCH(commun::commun_social, (pin)(unpin)(block)(unblock)(createreput)(changereput)(updatemeta)(deletemeta)(deletereput))
