...
GOLOS_IMAGE=$REPOSITORY/commun.contracts:$BUILDTYPE
echo "=== Extract Golos convertion scripts from $GOLOS_IMAGE"
rm -fR scripts || true
docker create --name extract-golos-scripts $GOLOS_IMAGE
docker cp extract-golos-scripts:/opt/commun.contracts/scripts scripts
...
echo "=== Deploy cyberway & commun.contracts"
docker run --rm --network cyberway_cyberway-net -ti $GOLOS_IMAGE /opt/commun.contracts/scripts/boot-sequence.py
