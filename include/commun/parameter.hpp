...
struct param_helper {
    ...
    static void check_params(const std::vector<T>& params, bool initialized) {
        ...
        check(params.size(), "empty params not allowed");
    ...
    static void check_immutables(const std::vector<T>& params) {
        for (const auto& param: params) {
            std::visit([](const parameter& p) {
                check(!p.immutable(), "can't change immutable parameter");
            }, param);
        }
    }
    ...
    static V set_parameters(std::vector<T> params, S& singleton, name ram_payer) {
        ...
        check(update || params.size() == state_type::params_count, "must provide all parameters in initial set");
        ...
        check(changed, "at least one parameter must change");    // don't add actions, which do nothing
    ...
    static T get_median(std::vector<T>& params) {
        auto l = params.size();
        check(l > 0, "empty params not allowed");
        T r = params[0];
        check(r.can_median(), "parameter can't be used as median");   // must not normally happen
        if (l > 1) {
    ...
    static bool have_majority(const std::vector<T>& params, size_t req_count, T& major) {
        auto l = params.size();
        check(l > 0, "empty params not allowed");
        // check(l >= req_count, "req_count is too large to reach");
        if (l < req_count) {
            return false; //req_count is too large to reach
        }
        check(l/2 + 1 <= req_count, "req_count is too small, it must be at least 50%+1");
        std::map<T,int> same;
        int max_count = -1;
        T max_value;
        for (const auto& p: params) {
...
struct set_params_visitor {
    ...
    bool set_param(const P& value, F field) {
        ...
        if (no_same) {
            check(changed, "can't set same parameter value");   // TODO: add parameter name to assert message
        }
        if (changed) {
            state.*field = value;
        }
