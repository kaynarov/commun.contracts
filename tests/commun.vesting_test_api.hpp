...
struct commun_vesting_api: base_contract_api {
    commun_vesting_api(golos_tester* tester, name code, symbol sym)
    ...
    action_result create_vesting(name creator) {
        return create_vesting(creator, _symbol, commun::config::control_name);
    }
...
//            _tester->link_authority(creator, _code, commun::config::invoice_name, N(retire));
        }
        return result;
    }
